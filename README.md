...
GitHubにpushされた学習資料の**PDFファイルのみ**を、**Google Apps Script (GAS)** が定期的にGoogle Driveに同期します。
...
| `output/learning_slides.pptx` | ❌ しない | 編集用ファイルはGitHubで管理 |
...
3. **PDFファイル抽出**:
   - `YYYY-MM-DD_トピック名/output/learning_slides.pdf` のみを対象
...
**シンプルな構造**: 各学習資料のPDFがフラットに並びます。ファイル名にトピック名が含まれているため、検索や整理が簡単です。

### 手動同期が不要な理由

- **自動化**: GASが定期実行されるため、手動でのアップロード操作は不要
- **一元管理**: GitHubが唯一の真実の情報源 (Single Source of Truth)
- **閲覧専用**: Google DriveはPDFの閲覧・共有専用として使用し、編集はGitHub側で行う
